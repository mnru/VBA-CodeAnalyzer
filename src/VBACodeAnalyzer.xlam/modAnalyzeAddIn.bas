Attribute VB_Name = "modAnalyzeAddIn"
Option Base 0
Function analyzeOtherBook(thisRibbon)
    
    pn = Application.GetOpenFilename("excel macro book,*.xlsm,all file,*.*", , "select workbook to analyze")
    
    If LCase(pn) = "false" Then Exit Function
  ...
  Call analyzeCode(thisRibon, True)
    ...
    analyzeOtherBook = True
End Function

Function analyzeCode(thisRibbon, Optional otherbook = False)
